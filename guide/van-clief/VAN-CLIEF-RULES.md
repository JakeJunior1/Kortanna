...
never `gh pr merge --delete-branch`. *(That ban is **local-only** — the resume-safe branch + worktree. The **remote** head branch is cleaned the opposite way, automatically: turn ON GitHub's **Automatically delete head branches** repo setting (`gh repo edit --delete-branch-on-merge`, on for every orchestrated remote) so a merged PR's remote branch is deleted **server-side the instant the worker merges** — no `--delete-branch` flag needed; the local branch + worktree are untouched and stay for the deliberate teardown above.)* *(A global `WorktreeCreate` hook still normalizes the placement of any
